    ...
    tbodyProd.innerHTML = lista.map((p, index) => {
      ...
      return `
        <tr>
          ...
          <td style="color:#0EA5E9; font-weight:800;">$${calcularVenta(p.costo, p.moneda)} CUP</td>
...
function renderApp() {
  ...
  if (dropdownProdCat) {
    dropdownProdCat.innerHTML = state.categorias.map(c => 
      `<div class="custom-option" onclick="seleccionarOpcion('prod-categoria', '${c.nombre}', '${c.nombre}', 'select-trigger-prod-categoria')">${c.nombre}</div>`
    ).join('');
  }
  ...
  if (dropdownProdTienda) {
    dropdownProdTienda.innerHTML = state.proveedores.map(p => 
      `<div class="custom-option" onclick="seleccionarOpcion('prod-tienda', '${p.nombre}', '${p.nombre}', 'select-trigger-prod-tienda')">${p.nombre}</div>`
    ).join('');
  }
...
async function guardarProveedor() {
  const nombre = document.=> item.id == idOIndice) || window.productosActuales[idOIndice];
      if (p) editarProducto(p);
    }
