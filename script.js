...
// Control Unificado de Sidebar Móvil
...
function filtrarYOrdenarProductos() {
  ...
  const tiendaInput = document.getElementById('filter-tienda');
  ...
  const tiendaSeleccionada = tiendaInput ? tiendaInput.value : '';
  ...
  let resultado = productosOriginales.filter(prod => {
    ...
    const coincideTienda = tiendaSeleccionada === "" || prod.tienda === tiendaSeleccionada;
    const coincideEstado = estadoSeleccionado === "" || prod.estado === estadoSeleccionado;

    return coincideNombre && coincideCat && coincideTienda && coincideEstado;
  });
...
function renderApp() {
  ...
  if (dropdownTienda) {
    const inputOculto = document.getElementById('filter-tienda');
    const valorActual = inputOculto ? inputOculto.value : '';

    let opcionesHTML = `
      <div class="custom-option ${valorActual === '' ? 'selected' : ''}" 
           onclick="seleccionarOpcion('filter-tienda', '', 'Todos los Proveedores', 'select-trigger-tienda')">
        Todos los Proveedores
      </div>`;

    opcionesHTML += state.proveedores.map(p => {
      const nombreProv = p.nombre || p.Nombre || '';
      return `
        <div class="custom-option ${valorActual === nombreProv ? 'selected' : ''}" 
             onclick="seleccionarOpcion('filter-tienda', '${nombreProv}', '${nombreProv}', 'select-trigger-tienda')">
          ${nombreProv}
        </div>`;
    }).join('');

    dropdownTienda.innerHTML = opcionesHTML;
  }
...
function seleccionarOpcion(inputId, valor, textoVisible, triggerId) {
  ...
  if (trigger) {
    ...
    // Feedback visual de filtro activo
    if (valor !== "") {
  ...
  if (inputId === 'filter-categoria' || inputId === 'filter-tienda' || inputId === 'filter-estado') {
    filtrarYOrdenarProductos();
  }
}
